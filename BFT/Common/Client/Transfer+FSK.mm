...
{
    NSLog(@"点付宝：刷卡器插入手机");
    [SVProgressHUD showSuccessWithStatus:@"刷卡器插入手机"];
...
{
    NSLog(@"点付宝：刷卡器已从手机拔出");
    [SVProgressHUD showErrorWithStatus:@"刷卡器已从手机拔出"];
}
...
{
    NSLog(@"点付宝：操作被中断");
    [SVProgressHUD showErrorWithStatus:@"操作被中断"];
}
...
-(void)onTimeout
{
    ...
    [SVProgressHUD showErrorWithStatus:@"操作超时"];
}
...
-(void) dataArrive:(vcom_Result*) vs  Status:(int) status
{
    if (status == -3) {
        ...
        [SVProgressHUD dismiss];
        [SVProgressHUD showErrorWithStatus:@"设备响应超时,请重试！"];
        
    } else if(status == -2) {
        NSLog(@"点付宝：设备没有连接手机！");
        [SVProgressHUD dismiss];
        [SVProgressHUD showErrorWithStatus:@"设备没有连接手机！"];
        
    } else if(status == -1) {
        NSLog(@"点付宝：接收数据格式错误！");
        
        [SVProgressHUD dismiss];
        [SVProgressHUD showErrorWithStatus:@"接收数据格式错误！"];
...
{
    [SVProgressHUD showWithStatus:@"设备已就绪，请刷卡" maskType:SVProgressHUDMaskTypeClear];
}
